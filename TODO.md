...
- [x] **T2.1**: 实现 `structure_parser.py` - 提取 Word 标题
- [x] **T2.2**: 实现 PDF 页码定位逻辑 - 生成 Map JSON
- [ ] **验证**: 运行解析脚本，确认章节页码对应正确
- [ ] **优化**: 优化解析的逻辑，现在关于章节定位还有很大问题

## 👁️ Phase 3: 视觉工具 (The Eyes)
- [x] **T3.1**: 实现 `visual_reader_tool.py` - PDF 特定页转图片并实现页面内容读取
- [x] **T3.2**: 集成 OpenBMB/MiniCPM-V-4模型 - 实现“看图说话”函数
- [x] **验证**: 手动调用函数读取第 5 页，检查描述是否准确
...
# Plan 2： gina_embedding_model 多模态嵌入，检索图片让vlm读取
